...
public class StudentDB {
	...
	public void voegToe(Student student) {
		if(student == null) throw new IllegalArgumentException("kan geen lege student toevoegen");
		Student st = vind(student.getNaam(),student.getVoornaam());
		if(st != null) throw new IllegalArgumentException("Kan geen twee dezelfde studenten toevoegen");
		klasLijst.add(student);
	}

	public void verwijder(String naam, String voornaam){
		klasLijst.removeIf(st -> st.getNaam().equals(naam) && st.getVoornaam().equals(voornaam));
	}
